...
/**
 * View body of WikiPagesRev
 *
 * @var string $versionId wiki_pages_revs.version_id
 */
	public function view_revision($versionId) {
		$post = $this->WikiPage->getRevision()->findByVersionId($versionId);
		$this->set('post', $post);
	}

/**
 * View diff between a page designated by versionId and the previous page
 *
 * @var string $versionId wiki_pages_revs.version_id
 */
	public function view_prev_diff($versionId) {
		$old = $this->WikiPage->getRevision()->findByVersionId($versionId);
		$this->WikiPage->id = $old['WikiPage']['id'];
		$diff = $this->WikiPage->diff(null, $versionId, array('limit' => 2));
		$this->set('diff', $diff);
	}

/**
 * View diff between a page designated by versionId and the latest page
 *
 * @var string $versionId wiki_pages_revs.version_id
 */
	public function view_latest_diff($versionId) {
		$old = $this->WikiPage->getRevision()->findByVersionId($versionId);
...
/**
 * If request by Ajax, return rendered page for preview.
 */
...
/**
 * Add comment
 * This method edits the WikiPage directly.
 */
	public function add_comment() {
		if ($this->request->is('post')) {
			...
			$wikiComment = "*$message - $name\n";
			$content = str_replace('{{comment}}', $wikiComment . '{{comment}}', $content);

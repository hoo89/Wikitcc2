...
class CategoriesController extends AppController {
	...
	public function index() {
		...
		$topPages = $this->Category->WikiPage->findAllByCategoryId(null);

		if ($this->request->is('requested')) {
			return $categories;
		} else {
			$this->set('categories', $categories);
			$this->set('top_pages', $topPages);
		}
	}
	...
	public function public_index() {
		...
		$topPages = $this->Category->WikiPage->find(
	...
	public function view($id = null) {
		...
		if ($this->request->is('post') && array_key_exists('id', $this->request->data['WikiPage'])){
			foreach ($this->request->data['WikiPage']['id'] as $wikiId => $selected) {
				if($selected){
					$this->Category->WikiPage->save(array('id' => $wikiId, 'category_id' => $this->request->data['WikiPage']['category_id'], 'modified' => false));
				}
			}
		}

		$categoryIds = $this->Category->children($id, false, 'id');
		$ids = array_map(
			function($item){
	...
	public function public_view($id = null) {
		...
		$categoryIds = $this->Category->children($id);
		$ids = array_map(
			function($item){
				if ($item['Category']['is_public']) return $item['Category']['id'];
			}, $categoryIds
		);
		$ids[] = $id;
